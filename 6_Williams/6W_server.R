#new
output$W_selectInputs <- renderUI({
  w <- ""
  
  if(input$W_obs!=0){
    isolate({
      w <- paste(w, textInput("W_y", "group_1", 
                              value = input[[sprintf("W_y",0)]]))
      for(i in 1:(input$W_obs)) {
        w <- paste(w, textInput(paste0("W_x", i, sep = ""), paste0("group_", i+1, sep = ""), 
                                value = input[[sprintf("W_x%d",i)]])) ## or value = "4 \n5 \n 6"
      }
...
W_X<-reactive({
  ...
  if (is.null(inFile)){#file or data
    ...
    if(input$W_obs!=0)#X and Y are definded
    {
      y_both <- str_trim(input$W_y, side = "both")
      ...
      col <- paste0("W_x", nobs) ## create a list of x name, x1 x2 x3...
  ...
  output$W_table <- renderTable({W_X()})
  
  W_compute <- function(){
    if(is.data.frame(W_X())){
      sink("result.txt")
      tbl <- W_X()
      a <- as.matrix(tbl)
      b <- t(a)
